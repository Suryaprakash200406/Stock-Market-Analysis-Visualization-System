...
<%
    ...
    List<Double> closeList = (List<Double>) request.getAttribute("closeList");
    List<Double> movingAvg = (List<Double>) request.getAttribute("movingAvg");
    List<Map<String, Object>> recentData =
            (List<Map<String, Object>>) request.getAttribute("recentData");

    String symbol = (String) request.getAttribute("symbol");
    String range = (String) request.getAttribute("range");

    double latestClose = (double) request.getAttribute("latestClose");
    double dailyChangePercent = (double) request.getAttribute("dailyChangePercent");
    double oneYearHigh = (double) request.getAttribute("oneYearHigh");
    double oneYearLow = (double) request.getAttribute("oneYearLow");
    double oneYearReturn = (double) request.getAttribute("oneYearReturn");
    Double latestMA = (Double) request.getAttribute("latestMA");
...
<div class="range-buttons">
    <a href="trends?symbol=<%=symbol%>&range=1M"
       class="<%=range.equals("1M")?"active":""%>">1M</a>
    <a href="trends?symbol=<%=symbol%>&range=6M"
       class="<%=range.equals("6M")?"active":""%>">6M</a>
    <a href="trends?symbol=<%=symbol%>&range=1Y"
       class="<%=range.equals("1Y")?"active":""%>">1Y</a>
    <a href="trends?symbol=<%=symbol%>&range=5Y"
       class="<%=range.equals("5Y")?"active":""%>">5Y</a>
    <a href="trends?symbol=<%=symbol%>&range=MAX"
       class="<%=range.equals("MAX")?"active":""%>">MAX</a>
</div>
...
<div class="card">
    <h4>20 Day MA</h4>
    <p>
        <%= latestMA != null ? "₹ " + String.format("%.2f", latestMA) : "N/A" %>
    </p>
</div>
...
new Chart(document.getElementById('trendChart'), {
    ...
    options: {
        ...
        scales: {
            x: {
                ticks: { color:'#e6edf3', maxTicksLimit: 10 },
                grid: { color:'#21262d' }
            },
            y: {
                ticks: { color:'#e6edf3' },
